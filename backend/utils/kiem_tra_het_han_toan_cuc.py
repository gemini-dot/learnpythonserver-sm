...
def kiem_tra_het_han(
    ...
    if thoi_gian_ket_thuc_trong_db and int(thoi_gian_hien_tai) > int(
        thoi_gian_ket_thuc_trong_db
    ):
        vi_tri.update_one(
            {str(vi_tri_can_xac_dinh): str(vi_tri_can_xac_dinh_bien)},
            {"$set": {str(ten_trang_thai_can_thay_doi): str(thay_doi)}},
        )
