...
from RestrictedPython.Eval import default_guarded_getitem, default_guarded_getiter # Đã thêm getiter
...
def ask_gemini_for_sandbox(system_instruction, user_text, doan_chat_truoc):
    ...
    try:
        ...
        instruction_msg = {"role": "user", "parts": [{"text": f"SYSTEM INSTRUCTION: {system_instruction}"}]}
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(temperature=0.4), # Tăng xíu nhiệt độ để nó nch tự nhiên hơn
        )
...
def handle_ai_code_generation(message_text, history):
    ...
    if code_match:
        ...
        final_reply = ask_gemini_for_sandbox(system_prompt_answer, final_answer_prompt, history)
        return final_reply
